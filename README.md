...
│   │   ├── licenses_controller.rb               # GET (paginated list) /companies/:company_id/licenses
│   │   ├── license_checkouts_controller.rb      # POST + GET (paginated list) /licenses/:license_id/checkouts
...
- `LicensesController#index` — read-only listing of all licenses for a
  company (`GET /companies/:company_id/licenses`), paginated via Kaminari.
  No service layer needed: this is a straight ActiveRecord query with no
  business logic.
...
### A hand-written audit log instead of a gem (`audited`/PaperTrail)

`LicenseAuditLog` is written explicitly by each service, not generated by
a change-tracking gem. The two well-known options for this —
[`audited`](https://github.com/collectiveidea/audited) and
[PaperTrail](https://github.com/paper-trail-gem/paper_trail) — both hook
into ActiveRecord callbacks, so they only fire when a record is actually
created/updated/destroyed. The most useful half of this audit log is the
rejected attempts — "no seats available," "license not found" — and
those never touch any model at all, so a callback-based gem would
silently miss exactly the cases worth auditing. Writing
`LicenseAuditLog.create!` directly inside the service, fed by the same
`Result` the service already returns to its caller, guarantees the log
can't disagree with the actual outcome.

### Kaminari for the listing endpoint, not a hand-rolled offset/limit

The checkout listing endpoint (`GET /licenses/:id/checkouts`) uses
Kaminari (`.page(...).per(...)`) rather than manual `offset`/`limit`
math. Unlike the audit log, there's no architectural mismatch here — a
"page N of size M" request maps directly onto what Kaminari does, so
there's no reason to maintain the equivalent logic by hand. Kaminari's
...
65 examples across three layers:
...
- **`spec/requests`** (24) — HTTP-level specs asserting status codes
  (201/200/409/404/422/500) end to end through the controllers, including
  listing/filtering/pagination on the checkouts and licenses endpoints and
  the `rescue_from` error-handling paths.
...
{"message":"License allocated successfully","checkout":{"id":7,"license_id":1,"user_id":42,"status":"active","checked_out_at":"2026-06-21T18:00:00.000Z","checked_in_at":null}}
```
...
### List licenses for a company (paginated)

```bash
curl -i "http://localhost:3000/companies/1/licenses?page=1&per_page=20"
```

```
HTTP/1.1 200 OK

{"licenses":[{"id":1,"name":"Toon Boom Harmony 21 — Floating License","max_seats":2,"active_seats_count":0,"license_key":null}],"page":1,"per_page":20,"total_count":1}
```

Returns 404 if the company does not exist. `per_page` is clamped to 1–100 (default 20).

## 8. CI

GitHub Actions (`.github/workflows/ci.yml`) runs on every push to `main`
and on every pull request (skipping draft PRs, running again once marked
ready for review):
...
3. `bundle exec rspec` — all 65 examples.
